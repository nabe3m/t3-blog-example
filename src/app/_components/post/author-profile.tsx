...
export function AuthorProfile({ authorId, authorName, authorImage }: AuthorProfileProps) {
	// authorIdが有効な場合のみクエリを実行
	const { data: profile, isLoading: profileLoading, error: profileError } = api.user.getById.useQuery(
		{ userId: authorId }, 
		{ 
			enabled: !!authorId && authorId.length > 0
		}
	);
	const { data: stats, isLoading: statsLoading, error: statsError } = api.user.getStats.useQuery(
		{ userId: authorId },
		{ 
			enabled: !!authorId && authorId.length > 0
		}
		...
		<div className="bg-gray-50 rounded-lg p-6 mt-8 border border-gray-200">
			...
			<div className="flex items-start space-x-4">
				...
				<div className="flex-1">
					...
					) : profile && authorId && authorId.length > 0 ? (
						<p className="text-gray-500 mb-4 text-sm italic">
							SNSリンクが設定されていません
						</p>
					) : null}

					{/* ユーザーアーカイブへのリンク - 常に表示（IDがある場合） */}
					{authorId && authorId.length > 0 ? (
						<Link 
							href={`/users/${authorId}`}
